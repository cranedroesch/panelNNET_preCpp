...
#I'm trying to calculate prediction intervals for a neural network.  Like any nonlinear model that has a writeable parametric structure, approximate inference is available via a linear taylor expansion.  See [this][1] paper, for example.  This involves computing the Jacobian -- the matrix of partial derivatives of $\hat y$ with respect to the parameters.

#In the one-outcome regression case, a neural net with $L$ hidden layers is
#$$
#y = \gamma_1 + V_1\Gamma_1 + \epsilon\\
#V_1 = a(\gamma_2 + V_2\Gamma_2)\\
#V_2 = a(\gamma_3 + V_3\Gamma_3)\\
#\vdots\\
#V_L = a(\gamma_L + \mathbf{X}\Gamma_L)\\
#$$
#alternatively:
#$$
#y = \gamma_1 + a(a(a(...a(\mathbf{X}\Gamma_L)\Gamma_{L-1})\Gamma_{L-2})...) + \epsilon
#$$


#where 

# - $y$ is the $N\times 1$ outcome 
# - $\mathbf{X}$ is the $N\times P_L$ data
# - $\Gamma_L$ is a $P_{L} \times P_{L-1}$ matrix of weights 
# - $\gamma_L$ is a bias/intercept term for the $L$th layer
# - $\epsilon$ is the mean-zero error 
# - $a()$ is the activation function

#The chain rule gives me the following partial derivatives of the fitted model:
#$$
#\frac{\partial \hat y}{\partial \Gamma_L} = \left(\displaystyle\prod_{2:L}a'\left(V_L\Gamma_L\right)\Gamma_{L-1}\right)\circ V_L
#$$

#For the first layer, we simply have:
#$$
#\frac{\partial \hat y}{\partial \Gamma_1} = V_1
#$$

#Now, the matrix dimensions on the above are not conformable.  This makes sense because the Jacobian is a tensor -- each element in the matrix is indexed by $i$, the $L$th layer (node emanating from), and the $L-1$th layer (node going to).

#So how do I actually go about turning the above into something that I can use to calculate the elements of the Jacobian matrix?  

#Here is an example in R, using [panelNNET][2], which is an experimental package that I've written for neural networks applied to panel data.
...
#    architecture <- c(8:3)
#    pl <- NULL
#    #Batch gradeint descent to start
#    pnn <- panelNNET(y[r], Z[r,], hidden_units = architecture
#      , fe_var = id[r], maxit = 1000, lam = lam
#      , time_var = time[r], param = P[r,, drop = FALSE],  verbose = TRUE
#      , gravity = 1.01, convtol = 1e-3, activation = 'lrelu', inference = FALSE
#      , start_LR = .01, parlist = pl, OLStrick = FALSE
#      , initialization = 'enforce_normalization'
#    )
#    #Make more progress using minibatch by jolting out of minima
#    pnn <- panelNNET(y[r], Z[r,], hidden_units = architecture
#      , fe_var = id[r], maxit = 1000, lam = lam
#      , time_var = time[r], param = P[r,, drop = FALSE],  verbose = TRUE
#      , gravity = 1.01, convtol = 1e-3, activation = 'lrelu', inference = FALSE
...
#    )
#    #Use the "OLS trick" -- closed-form solution to top layer of network, treated as a ridge regression in derived variables
...
#The weights are in `pnn$parlist` (`beta` are the top-level $\Gamma$'s and `beta_param` are coefs on terms not represented as part of the network.

#    1> lapply(pnn$parlist, dim)
#    [[1]]
#    [1] 6 8
#    
#    [[2]]
...
#and the hidden layers are in `pnn$hidden_layers`

#    1> lapply(pnn$hidden_layers, dim)
#    [[1]]
#    [1] 900   8
#    
#    [[2]]
...
#The parameter matrices follow the convention that the bias is the first row.

#So, given this set of parameters and hidden layers, how can I calculate the Jacobian?  

#A previous commenter suggested that I should already have it from backprop.  But I don't, because I vectorize my backprop, rather than treating it like a tensor.  This is possible because I want the average 

#  [1]: https://www.esa.espci.fr/sites/www.esa.espci.fr/IMG/pdf/2000conf.pdf
#  [2]: https://github.com/cranedroesch/panelNNET
