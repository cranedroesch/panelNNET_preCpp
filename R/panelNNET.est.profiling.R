...
time <- 1:N/100
id <- (1:N-1) %/% 100 +1
y <- x*sin(x)+ rnorm(N, sd = 9) +time +id
plot(x,y)
library(microbenchmark)
#y = y[r]
X = matrix(x)
hidden_units = c(1000, 1000, 1000, 1000)
fe_var = factor(id)
maxit = 1000
lam = .01
...
print(paste(round(proc.time()[3] - PT[3],3), 'getting updated gradients'))
...
print(paste(round(proc.time()[3] - PT[3],3), 'calculating updates'))
      #Update parameters from update list
PT <- proc.time()
      parlist <- mapply('-', parlist, updates)
print(paste(round(proc.time()[3] - PT[3],3), 'applying updates'))
PT <- proc.time()
      ...
      if (is.null(fe_var)){hlayers[[i]] <- cbind(1, hlayers[[i]])}#add intercept if no FEs
print(paste(round(proc.time()[3] - PT[3],3), 'updating hidden layers'))
PT <- proc.time()
      #update yhat
      if (!is.null(fe_var)){
        Zdm <- demeanlist(hlayers[[i]], list(fe_var))
...
print(paste(round(proc.time()[3] - PT[3],3), 'updating yhat'))
      if (verbose == TRUE){
        writeLines(paste0(
            "*******************************************\n"
          , tag, "\n"
